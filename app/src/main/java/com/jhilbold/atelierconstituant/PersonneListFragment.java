...
/**
 * A list fragment representing a list of Personnes. This fragment also supports tablet devices by
 * allowing list items to be given an 'activated' state upon selection. This helps indicate which
 * item is currently being viewed in a {@link PersonneDetailFragment}.
 * <p/>
 * Activities containing this fragment MUST implement the {@link Callbacks} interface.
 */
public class PersonneListFragment extends ListFragment
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if(getActivity().getIntent().hasExtra("atelierID"))
		{
			Atelier a = Ateliers.ITEM_MAP.get(getActivity().getIntent().getStringExtra("atelierID"));

			setListAdapter(new ArrayAdapter<Personne>(getActivity(), android.R.layout.simple_list_item_activated_1, android.R.id.text1, a.getInvites()));
		}
		else
		{
			// TODO: replace with a real list adapter.
			setListAdapter(new ArrayAdapter<Personne>(getActivity(), android.R.layout.simple_list_item_activated_1, android.R.id.text1, Personnes.ITEMS));
		}
	}
	...
	public void onListItemClick(ListView listView, View view, int position, long id)
	{
		...
		mCallbacks.onItemSelected(Personnes.ITEMS.get(position).getId());
	}
